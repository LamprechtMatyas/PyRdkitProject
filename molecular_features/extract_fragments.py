""""
Extract fragments for molecules in given SDF/SMILES files and save then into JSON.
...
def get_atom_code(atom: object, branch_subtract: int) -> int:
    # Constants;
...
def score_path(molecule: object, path: list, size: int) -> int:
    codes = [None] * size
    for i in range(size):
        if i == 0 or i == (size - 1):
            sub = 1
...
def extract_neighbourhood_fragments(molecule: object, size: int, options: dict, ecfp: bool) -> list:
...
def extract_fragments_from_molecule(molecule: object, types: list, options: dict) -> list:
...
def _read_configuration() -> Dict[str, int]:
...
def recursive_scan_for_input(path: str, recursive: bool, extension: str) -> List[str]:
...
def append_object_to_json(output_stream: TextIO, item: dict, holder: Dict[str, bool]):
